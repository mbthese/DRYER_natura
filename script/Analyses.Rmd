...
Steps to compute R2adj (Peres-Neto et al. 2006) - `varpart()` in vegan. A negative value is when the combination of variables explains less than when applied separately. It is also argued that they can be set to zero, although not really satisfying. Yet, quoted from GUSTA ME, "These are artifacts of the VP procedure which arise when certain relationships are present in the data. No solution is available to remove them meaningfully." (https://mb3is.megx.net/gustame/constrained-analyses/variation-partitioning). 
...
gmin <-ggplot(traits, aes(Name, gmin, color= Name))+
 ...
 scale_color_discrete(name= "Species")+
  ...
  ylim(0, 9)+
  ggtitle(expression(paste(italic("g")[min], ~ (mmol ~ m^2 ~ s^-1)))) +
...
#option2
PCA_organ<- ggarrange(PCA_leaf , PCA_root, labels = c("A-Leaf", "B-Root"), vjust = c(1,1), hjust = c(-1.5,-1.5),font.label = list(size = 20), ncol=2, nrow=1, legend = "bottom", common.legend = T)
PCA_organ

ggsave(filename = "Figure4.pdf", plot = PCA_organ, bg = "white", width = 12, height = 8, dpi = 600)
...
## drivers of alpha diversity (Table 1)
...
#eventhough anova(mod) is slightly significant, when we plot, the group dispersion is relatively the same except for some samples for away from the centroids. But now let's test if the compositions are different, as seen on the plot.

adonis2(formula = hill.beta.dist ~ as.factor(natura_clean$samples$organ), perm= 999)#Observing our results we can see that groups have significantly different compositions (p<0.05). 
...
p<- ggplot(data=df,aes(x=Group,y=Distance_to_centroid,colour=groups)) + 
  ...
  annotate(geom="text", x=1.5, y=1.5, label="Permutation test",
              color="black", size = 3)+
   annotate(geom="text", x=1.5, y=1.45, label= expression(italic(P) == 0.01),
              color="black", size = 3)+
   ylab('Distance \n to centroid') +
...
adonis2(hill.beta.dist~as.factor(leaf$samples$Name), permutations = 999)
...
pcoa_beta_leaf <-  ggplot() +
 ...
 # geom_segment(aes(x = PCoA1.x, y = PCoA2.x, yend = PCoA2.y, xend = PCoA1.y, group = group, col = group), betadisper_lines) +
  ...
  xlab('PCoA 1  10.0%') +
  ggtitle("Leaf") +
  theme(plot.title = element_text(color = "#6bbf59"), legend.position = 'bottom', legend.text = element_text(face= "italic", size = 12)) +
  labs(col = "Host species")
pcoa_beta_leaf  
...
leaf<- df %>%
 ...
 # annotate(geom="text", x=1, y=0.69, label="Permutation test",color="black")+
   annotate(geom="text", x=6.9, y=1.4, label=expression(italic(P) == 0.8),
              color="black", size =3)+
...
adonis2(hill.beta.dist~as.factor(root$samples$Name), permutations = 999)
...
pcoa_beta_root <- ggplot() +
  ...
  theme(legend.position = 'bottom') +
   ggtitle("Root")+
    theme(plot.title = element_text(color = "#E79F02"))+
...
root<- df %>%
  ...
  theme_classic(base_size = 12)+
 # annotate(geom="text", x=1, y=0.69, label="Permutation test", color="black")+
   annotate(geom="text", x=1.1, y=1.4, label=expression(italic(P) == 0.6),
              color="black", size=3)+
   theme(legend.position = "none", axis.title.x =element_blank(), axis.text.x = element_blank())+
  scale_color_manual(values = group_colors_sans_tachi)
...
Beta_option2 <- ggarrange(Beta_all, Beta_leaf, Beta_root,legend_plot, ncol = 1, nrow = 4, heights = c(1,1,1, 0.5), widths = c(0.5, 0.5,0.5,0.8))
Beta_option2
...
##drivers of beta diversity (Appendix S18 fungi)
...
all_trait <- data.frame(traits = c("Host species", "gmin", "LSWC", "LT", "SLA", "SD", "Total_LA", "MEM1", "MEM2", "MEM3", "MEM4", "All variables"))
...
```{r}
# Create a data frame for plotting
rsquare_df_root_ITS <- data.frame(Trait = c("Host species", "SRL", "RD", "RTD", "MEM1", "MEM2", "MEM3", "MEM4", "All variables"), Adj.R.squared = c("", "", "", "", "","", "", "", ""))

# Plot the estimates using ggplot2
root_fungi <-ggplot(rsquare_df_root_ITS, aes(x = Trait, y = Adj.R.squared)) +
  labs(x = "", y = expression(paste("Adjusted",~italic(R^2)))) +
  ggtitle("Root fungi")+
  theme_classic(base_size = 15) +
  coord_flip()
```
...
## drivers of alpha diversity (Table 2) 
...
adonis2(hill.beta.dist~as.factor(natura_clean$samples$organ), perm= 999)
...
p<- ggplot(data=df,aes(x=Group,y=Distance_to_centroid,colour=groups)) + 
  ...
  annotate(geom="text", x=1.5, y=1.5, label="Permutation test",
              color="black", size = 3)+
   annotate(geom="text", x=1.5, y=1.43, label=expression(italic(P) == 0.07),
              color="black", size = 3)+
   ylab('Distance \n to centroid') +
   theme(legend.position = "none", axis.title.x =element_blank())  +
  scale_color_manual(values = c(leaf = "#6bbf59", root = "#E79F02"))
...
TK.leaf <- group_by(df, Group) %>%
  summarise(mean = mean(Distance_to_centroid),
            quant = quantile(Distance_to_centroid, probs = 0.90)) %>%
  arrange(desc(mean))
TK.leaf$letters <- letters.leaf$Letters
TK.leaf

TK.leaf$x_axis <- c(6,4,2,3,1,5,7)
TK.leaf$y_axis <- c(1,1,1.13,1,0.95,1,1.05)
...
leaf<- df %>%
  ...
  theme_classic(base_size = 12)+
  #annotate(geom="text", x=1, y=0.69, label="Permutation test",color="black")+
  geom_text(data = TK.leaf, aes(label = letters, x = x_axis, y = y_axis),
            vjust = -0.2, hjust = 1, size = 3, color = "black") +
  annotate(geom="text", x=1.1, y=1.1, label=expression(italic(P) == 0.03),
              color="black", size =3)+
  theme(legend.position = "none", axis.title.x =element_blank(), axis.text.x = element_blank())
leaf
...
root<- df %>%
  ...
  geom_boxplot(alpha=0.5) + 
    ylab('Distance \n to centroid') +
  theme_classic(base_size = 12)+
   annotate(geom="text", x=1.1, y=1.1, label=expression(italic(P)==0.4),
              color="black", size=3)+
   theme(legend.position = "none", axis.title.x =element_blank(), axis.text.x = element_blank())
...
## drivers of beta diversity (Appendix S15 bacteria)
...
#traits
names(leaf$samples)
env <- leaf$samples[,c(14,40:43,48:53)] #only leaf traits and spatial predictors
# Prior to constructing the model, root functional trait variables were mean-centered and scaled to unit variance.
names(env)
env.scale <- scale(env[,-c(1)], center = TRUE, scale = TRUE) #scale traits not the indices
env.scale <-as.data.frame(env.scale)
...
# Create a data frame for plotting
rsquare_df_leaf <- data.frame(Trait = c("Host species", "gmin", "LSWC", "LT", "SLA", "SD", "Total_LA", "MEM1", "MEM2", "MEM3", "MEM4", "All variables"), Adj.R.squared = c("", "", "","", "", "","", "", "","", "", ""))
...
#traits
names(root$samples)
env <- root$samples[,c(14,40:43,48:50)] #only root traits and spatial predictors
# Prior to constructing the model, root functional trait variables were mean-centered and scaled to unit variance.
names(env)
env.scale <- scale(env[,-c(1)], center = TRUE, scale = TRUE) #scale traits not the indices
env.scale <-as.data.frame(env.scale)
...
all_trait <- data.frame(traits = c("Host species", "SRL", "RD", "RTD", "MEM1", "MEM2", "MEM3", "MEM4", "All variables"))
...
#Plot all (Appendix S18 all)
```{r}
library(patchwork)

beta_drivers_plot <- leaf_fungi + Leaf_bacteria + root_fungi + root_bacteria

ggsave(filename = "beta_drivers2.jpeg", plot= beta_drivers_plot, bg= "white", width = 14, height = 12, dpi = 600)
...
Normalized stochasticity ratio (NST) was further applied to identify the bacterial community assembly processes, where an index developed with 50% as the distinct point between more stochastic (>50%) assembly and more deterministic (<50%) (Ning et al. 2019). This analysis was performed in the R v3.6.3 with the “NST” package (Ning et al. 2019).
```{r}
...
##fungi analyses root vs. leaves
```{r}

#load data fungi and match IDs
load("D:/Sophie_pipeline/obitools/resources/Metabarlist_natura_clean_ITS2_traits.Rdata")
fungi <- natura_clean
comm.fungi <- (fungi$reads)
sample.fungi = fungi$pcrs[,c(1,4)]
sample.fungi.2 <- subset(sample.fungi, select=c(organ))

# calculate tNST, 'jaccard' and 'bray' are preferred according to Ning et al. 2019
nst <- tNST(comm = comm.fungi,group = sample.fungi.2,dist.method = "bray",rand = 1000, output.rand = T) #did it with 1000 as recommanded, here set 20 to save test time. Have to have output.rand= T to do the boootstrapping after.

#save
prefixi2 = "mydata_fungi_rand1000" # prefix in the output filenames
save(nst,file = paste0(prefixi2,".tNST.rda")) # save tNST output in R data format
write.table(nst$index.grp,file = paste0(prefixi2,".tNST.summary.csv"), quote = FALSE,sep = ",")
write.table(nst$index.pair.grp,file = paste0(prefixi2,".tNST.pairwise.csv"),quote = FALSE,sep = ",")
...
#data:  nst_boot$detail$NST.boot$leaf and nst_boot$detail$NST.boot$root
# t = -37.41, df = 153.86, p-value <
# 2.2e-16
...
# calculate tNST, 'jaccard' and 'bray' are preferred according to Ning et al. 2019
nst <- tNST(comm = comm.bacteria, group = sample.bacteria.2,dist.method = "bray",rand = 1000, output.rand = T) #did it with 1000 as recommanded, here set 20 to save test time.

#save
prefixi2 = "mydata_bacteria_rand1000" # prefix in the output filenames
save(nst,file = paste0(prefixi2,".tNST.rda")) # save tNST output in R data format
write.table(nst$index.grp,file = paste0(prefixi2,".tNST.summary.csv"), quote = FALSE,sep = ",")
write.table(nst$index.pair.grp,file = paste0(prefixi2,".tNST.pairwise.csv"),quote = FALSE,sep = ",")
...
# data:  nst_boot$detail$NST.boot$leaf and nst_boot$detail$NST.boot$root
# t = -0.2086, df = 173.73, p-value =
# 0.835
# alternative hypothesis: true difference in means is not equal to 0
# 95 percent confidence interval:
#  -0.005810161  0.004699410
# sample estimates:
# mean of x mean of y 
# 0.8473098 0.8478652 
...
#varpart
In order to assess the factors responsible for differences in beta diversity, separately for bacteria and fungi and for leaf and root organs, variance partitioning analyses were carried out on the raw-OTU table (function varpart in the vegan package); the explanatory variables that significantly explained variation in endophyte community composition were determined by forward selection using the ordistep function in vegan. 
...
#leaf
MEM <- env.scale[,c(1,2,3,4)]
leaf_traits <- env.scale[,c(5:10)]

bacteria.leaf <- varpart(leaf_dist$, ~ Name,
                         ~norm_gmin + norm_LSWC + norm_LT + norm_SLA + norm_StomatalDensity + norm_TotalLeafArea,  
                         ~ MEM1 + MEM2 + MEM3 +MEM4,
                         data = env.scale)
bacteria.leaf
plot(bacteria.leaf, digits = 2, Xnames = c('Species', 'Leaf traits', 'Spatial'), bg = c('tomato', 'green', 'navy'))

rda.all<-rda(leaf$reads~Name + norm_gmin + norm_LSWC + norm_LT + norm_SLA + norm_StomatalDensity + norm_TotalLeafArea+ 
                        MEM1 + MEM2 + MEM3 +MEM4, data=env.scale)
rda.all
set.seed(1234); step.forward <- ordistep(rda(leaf$reads ~ 1, data=env.scale), scope=formula (rda.all), direction="forward", pstep=1000)

step.forward.leaf.bacteria <- step.forward
...
names(traits_norm)
root$samples <- left_join(root$samples, traits_norm %>% dplyr::select(code, norm_SRL, norm_RootTissueDensity,norm_RootDiameter)) 
...
bacteria.root<- varpart(root$reads, ~ Name,
                         ~ norm_SRL+ norm_RootTissueDensity+norm_RootDiameter,
                         ~ MEM1 + MEM2 + MEM3 +MEM4,
                         data = env.scale)
plot(bacteria.root, digits = 2, Xnames = c('Species', 'Root traits', 'Spatial'), bg = c('tomato', 'brown', 'navy'))

rda.all.root<-rda(root$reads~Name + norm_SRL+ norm_RootTissueDensity+norm_RootDiameter+ 
                        MEM1 + MEM2 + MEM3 +MEM4, data=env.scale)
rda.all.root
set.seed(1234); step.forward <- ordistep(rda(root$reads ~ 1, data=env.scale), scope=formula (rda.all.root), direction="forward", pstep=1000)

step.forward.root.bacteria <- step.forward
```
...
df.venn.leaf<- data.frame(x = c(3, 1, 2),y = c(1, 1,2.8),labels = c('Host species\n0.003', 'Leaf traits\n0.018','Spatial\n'))

#leaf
leaf.bacteria <- ggplot(df.venn.leaf, aes(x0 = x, y0 = y, r = 1.5, fill = labels)) + geom_circle(alpha = .6, size = 1, colour = "#3f3a3a",show.legend = FALSE) +
                  coord_fixed()+
                  annotate("text", x = df.venn.leaf$x , y = df.venn.leaf$y,label=df.venn.leaf$labels ,size = 6) +
                  scale_fill_manual(values = c("#f2e8cf", #host
                                               "#76c893", #leaf
                                               "#219ebc")) +#spatial +
                  theme_void()+
                  annotate("text", x = 3.5 , y =-0.8,label="Residuals: 0.982" ,size = 4.5)

                  
leaf.bacteria

#root
df.venn.root<- data.frame(x = c(3, 1, 2),y = c(1, 1,2.8),labels = c('Host species\n0.092 *', 'Root traits\n','Spatial\n'))

root.bacteria <- ggplot(df.venn.root, aes(x0 = x, y0 = y, r = 1.5, fill = labels)) + geom_circle(alpha = .6, size = 1, colour = "#3f3a3a",show.legend = FALSE) +
                  coord_fixed()+
                  annotate("text", x = df.venn.root$x , y = df.venn.root$y,label=df.venn.root$labels ,size = 6) +
#                  scale_fill_manual(values = c("#82662a","#bea773","#daba73")) +#old colors: brownish
                  scale_fill_manual(values = c("#f2e8cf",
                                               "#dda15e",
                                               "#219ebc")) +
                  theme_void()+
                  annotate("text", x = 3.5 , y =-0.8,label="Residuals: 0.902" ,size = 4.5)

root.bacteria

#together
bacteria_vartpart <- ggarrange(leaf.bacteria, root.bacteria, labels = c("Leaf bacteria", "Root bacteria"), nrow=1, ncol=2)
```
...
names(leaf$samples)
env <- leaf$samples[,c(13,39:42,46:51)] #only leaf traits and spatial predictors
# Prior to constructing the model, root functional trait variables were mean-centered and scaled to unit variance.
names(env)
env.scale <- scale(env[,-c(1)], center = TRUE, scale = TRUE) #scale traits not the indices
env.scale <-as.data.frame(env.scale)
...
fungi.leaf <- varpart(leaf$reads, ~ Name,
                         ~norm_gmin + norm_LSWC + norm_LT + norm_SLA + norm_StomatalDensity + norm_TotalLeafArea,  
                         ~ MEM1 + MEM2 + MEM3 +MEM4,
                         data = env.scale)
fungi.leaf
plot(fungi.leaf, digits = 2, Xnames = c('Species', 'Leaf traits', 'Spatial'), bg = c('tomato', 'green', 'navy'))
...
step.forward.leaf.fungi <- step.forward
...
names(root$samples)
env <- root$samples[,c(13,39:42,46:48)] #only root traits and spatial predictors
# Prior to constructing the model, root functional trait variables were mean-centered and scaled to unit variance.
names(env)
env.scale <- scale(env[,-c(1)], center = TRUE, scale = TRUE) #scale traits not the indices
env.scale <-as.data.frame(env.scale)
...
fungi.root<- varpart(root$reads, ~ Name,
                         ~ norm_SRL+ norm_RootTissueDensity+norm_RootDiameter,
                         ~ MEM1 + MEM2 + MEM3 +MEM4,
                         data = env.scale)
plot(fungi.root, digits = 2, Xnames = c('Species', 'Root traits', 'Spatial'), bg = c('tomato', 'brown', 'navy'))

rda.all.root<-rda(root$reads~Name + norm_SRL+ norm_RootTissueDensity+norm_RootDiameter+ 
                        MEM1 + MEM2 + MEM3 +MEM4, data=env.scale)
rda.all.root
set.seed(123); step.forward <- ordistep(rda(root$reads ~ 1, data=env.scale), scope=formula (rda.all.root), direction="forward", pstep=1000)

step.forward.root.fungi <- step.forward
```
...
#leaf
leaf.fungi <- ggplot(df.venn.leaf, aes(x0 = x, y0 = y, r = 1.5, fill = labels)) + geom_circle(alpha = .6, size = 1, colour = "#3f3a3a",show.legend = FALSE) +
                  coord_fixed()+
                  annotate("text", x = df.venn.leaf$x , y = df.venn.leaf$y,label=df.venn.leaf$labels ,size = 6) +
                  scale_fill_manual(values = c("#f2e8cf", #host
                                               "#76c893", #leaf
                                               "#219ebc")) +#spatial +
                  theme_void()+
                  annotate("text", x = 3.5 , y =-0.8,label="Residuals: 0.953" ,size = 4.5)
...
root.fungi <- ggplot(df.venn.root, aes(x0 = x, y0 = y, r = 1.5, fill = labels)) + geom_circle(alpha = .6, size = 1, colour = "#3f3a3a",show.legend = FALSE) +
                  coord_fixed()+
                  annotate("text", x = df.venn.root$x , y = df.venn.root$y,label=df.venn.root$labels ,size = 6) +
#                  scale_fill_manual(values = c("#82662a","#bea773","#daba73")) +#old colors: brownish
                  scale_fill_manual(values = c("#f2e8cf",
                                               ...
                                               "#219ebc")) +
                  theme_void()+
                  annotate("text", x = 3.5 , y =-0.8,label="Residuals: 1" ,size = 4.5)

root.fungi

#together
fungi_vartpart <- ggarrange(leaf.fungi, root.fungi, labels = c("Leaf fungi", "Root fungi"), nrow=1, ncol=2)
```


# END

#NMDS: https://www.frontiersin.org/articles/10.3389/fbioe.2023.1197175/full#h6
