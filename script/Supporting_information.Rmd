...
analyses_sol <- read_excel("D:/Sophie_pipeline/obitools/resources/DRYER_natura/analyses_sol.xls", sheet = "Feuil2")

analyses_sol <- analyses_sol[-c(7:11), ] 
analyses_sol <- analyses_sol[-c(19:20), ] 
analyses_sol <- analyses_sol %>% dplyr::select(-Greenhouse) %>% rename(Value = Paracou)



analyses_sol_table <- gt(analyses_sol)

analyses_sol$Variable[9] <- paste0(analyses_sol$Variable[9], 
                                footnote_marker_symbol(1))

analyses_sol$Value <- as.numeric(analyses_sol$Value) %>% round(digits = 2)
analyses_sol<- analyses_sol %>% drop_na(Value)

kbl(analyses_sol, escape =F, padding = -1L) %>%
  kable_classic_2(full_width = F) %>%
#   kable_styling(font_size = 20) %>%
 row_spec(0, bold = TRUE, font_size = 15) %>%
  footnote(general = "Soils were prepared according to the NF ISO 11464 standard, Cirad, Montpellier.",
           symbol = c("For available phosphorous, the Olsen method was used, the best method for acid soils.")) %>%
 save_kable(
...
## Appendix S2: Phylogeny of the seven selected tree species
...
paracou <- read_excel("resources/Paracou_database20210830.xlsx") %>% 
...
save(tree, file = "/Sophie_pipeline/obitools/results/phylogeny_subset.Rdata")
load("/Sophie_pipeline/obitools/results/phylogeny_subset.Rdata")

#build tree without legend to get the legend without 'na'
plot_tree_legend <- fortify(tree$scenario.3) %>% 
  mutate(species = gsub("_", " ", label)) %>% 
  left_join(paracou) %>%
  mutate(taxon = gsub("_", " ", taxon)) %>%
  ggtree(aes(col = taxon), layout="circular") +   
  geom_tiplab2(aes(alpha = !is.na(taxon), size = !is.na(taxon))) +
  theme_tree(legend.position = "bottom",  axis.text = element_text(size = 20), legend.text = element_text(face = "italic", size = 20), legend.title = element_text(size = 22)) +
  scale_alpha_manual(values = c(0.2, 1), guide = "none") +  # Hide alpha legend
  scale_size_manual(values = c(1, 4), guide = "none") +     # Hide size legend
  labs(color = "Taxa")

plot_tree_legend 

p <- rotate_tree(plot_tree_legend, -20) + xlim(NA, 200)


#save and choose saving format
ggsave(file="phylo_subset.svg", plot=p, width=15, height=10)
ggsave(file="phylo_subset.jpg", plot=p, width=15, height=10)
```
...
p <-  traits %>%
  ...
  mutate(Trait = dplyr::recode(Trait, "NbLeaves" = "Number~of~leaves", "Height"= "Height~(cm)", "StemDiameter"="Stem~diameter~(mm)", "RootShoot"= "Root~to~shoot~ratio")) %>% 
...
##Appendix S8: Taxonomic assignment
```{r}
#data fungi
load("./results/Metabarlist_natura_clean_ITS2.Rdata")
...
leaf_fungi_taxo$Assignment <- rownames(leaf_fungi_taxo)
leaf_fungi_taxo <- leaf_fungi_taxo %>% 
  relocate(Assignment, .before = nbr_fungi_leaf)
...
root_fungi_taxo$Assignment <- rownames(root_fungi_taxo)
root_fungi_taxo <- root_fungi_taxo %>% 
  relocate(Assignment, .before = nbr_fungi_root)
...
leaf_bacteria_taxo$Assignment <- rownames(leaf_bacteria_taxo)
leaf_bacteria_taxo <- leaf_bacteria_taxo %>% 
  relocate(Assignment, .before = nbr_bacteria_leaf)
...
root_bacteria_taxo$Assignment <- rownames(root_bacteria_taxo)
root_bacteria_taxo <- root_bacteria_taxo %>% 
  relocate(Assignment, .before = nbr_bacteria_root)
...
final_assignation$Assignment <- rownames(final_assignation)
final_assignation <- final_assignation %>% 
  relocate(Assignment, .before = `bacteria leaf assignation(%)`)
...
final_assignation[1, 1] <- '"Kingdom"'

kbl(final_assignation, escape =F, padding = -1L, row.names = FALSE) %>%
  kable_classic_2(full_width = F) %>%
 kable_styling(font_size = 20) %>%
 row_spec(0, bold = TRUE, font_size = 15) %>%
 save_kable(
...
##Appendix S9: Major fungal phyla detected in the roots and leaves of the seven tropical tree seedlings.
...
##Appendix S10: Top 20 fungal orders detected in the roots and leaves of the seven tropical tree seedlings.
...
##Appendix S11:Top 20 fungal genera detected in the roots and leaves of the seven tropical tree seedlings.
```{r}
load("./results/Metabarlist_natura_clean_ITS2.Rdata")
leaf <- subset_metabarlist(natura_clean, table = "samples",
                          indices = natura_clean$samples$organ == "leaf")
...
##Appendix S12: Major bacterial phyla detected in the roots and leaves of the seven tropical tree seedlings.
...
##Appendix S13: Top 20 bacterial orders detected in the roots and leaves of the seven tropical tree seedlings.
...
##Appendix S14: Top 20 bacteria genera detected in the roots and leaves of the seven tropical tree seedlings.
```{r}
load("./results/Metabarlist_natura_clean_16S.Rdata")
leaf <- subset_metabarlist(natura_clean, table = "samples",
                          indices = natura_clean$samples$organ == "leaf")
...
ggsave("./results/Genus_bacteria.jpeg", Genus, width = 8, height = 10)
```

##Appendix S15: Diversity of seven tropical tree seedlings.
...
alpha_16S_plot_leaf <- leaf$samples  %>%
  ...
  scale_fill_manual(
    ...
    root = "#E79F02"))+
 ylab(expression(paste("Bacteria / \nEffective number of species")))+
  theme_minimal(base_size = 15)+
    geom_text(aes(x = 1, y = 150), label = expression(italic(P) == 0.8), color = "black", size = 6)+
...
alpha_16S_plot_root <- root$samples %>%
  ...
  aes(x = name, y = Shannon, fill = organ) +
  geom_boxplot() +
  scale_fill_manual(values = c(leaf = "#6bbf59", root = "#E79F02")) +
  theme_minimal(base_size = 15) +
  geom_text(aes(x = 1, y = 110), label = expression(italic(P) == 0.4), color = "black", size = 6) +
...
alpha_ITS_plot_leaf <- leaf$samples  %>%
  ...
  scale_fill_manual(
    values = c(leaf = "#6bbf59",
    root = "#E79F02"))+
  ylab(expression(paste("Fungi / \nEffective number of species")))+ #\n jump line
  theme_minimal(base_size = 15)+
  geom_text(aes(x = 1, y = 250), label = expression(italic(P) == 0.9), color = "black", size = 6)+
...
alpha_ITS_plot_root <- root$samples  %>%
  ...
  theme_minimal(base_size = 20)+
    geom_text(aes(x = 1.05, y = 100), label = expression(italic(P) == 0.03), color = "black", size = 6)+
...
# include extra margin space
alpha_ITS_plot_leaf <- alpha_ITS_plot_leaf +
  theme(plot.margin = unit(c(1, 1, 1.5, 1), "cm"))

alpha_ITS_plot_root <- alpha_ITS_plot_root +
  theme(plot.margin = unit(c(1, 1, 1.5, 1), "cm"))

alpha_16S_plot_leaf <- alpha_16S_plot_leaf +
  theme(plot.margin = unit(c(1, 1, 1.5, 1), "cm"))

alpha_16S_plot_root <- alpha_16S_plot_root +
  theme(plot.margin = unit(c(1, 1, 1.5, 1), "cm"))

# Combine 
alpha_plot <- ggarrange(
  alpha_ITS_plot_leaf,
  alpha_ITS_plot_root,
  alpha_16S_plot_leaf,
  alpha_16S_plot_root,
  ...
  nrow = 2,
  legend = "none",
  label.y = c(1, 1, 1.15, 1.15),
  font.label = list(size = 18)
)

ggsave("./results/SI_Hill_diversity.jpeg", alpha_plot, width = 12, height =8)
```

##Appendix S16 : Trait variation in 72 individuals among seven SF seedlings species.

**Trait variation (Appendix S13)** in main script Analyses.

##Appendix S17 : Total contribution of variables to PCA axes 1, 2, 3 and 4.
...
##Appendix S18 : Drivers of composition.
