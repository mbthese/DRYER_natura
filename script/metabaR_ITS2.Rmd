...
Taxa assignation with dada2 package and get reference database here: https://unite.ut.ee/repository.php 
...
samples  <- file_samples %>%
  ...
  separate(Species, into = c("Species", "VXX"), sep = ";")%>%
    dplyr::select(-tag, -primer_forward, -primer_reverse, -VX, -VXX)
...
#read
colnames(file_obitab)
read <- file_obitab %>% dplyr::select(c(1,21:183))
read_t <- t(read[,-1])
...
assign <- read.csv("E:/UNITE_assignation/taxa.ITS.csv", header = TRUE)
...
dim(assign_motus) #[1] 13255    10
dim(motus) #[1] 13255     3
dim(assign) #[1] 13105     8
...
#character(0)
# previously [1] "DRYER_natura_NA_16S_control_extraction_5" "DRYER_natura_NA_16S_control_PCR_21"      
...
pcrs     2039413    13255  12511.74 8469.602  473.8712 358.6601
samples  2023631    13222  15566.39 6588.521  579.6538 320.0792
...
```{r}
#Flag MOTUs corresponding to target (TRUE) vs. non-target (FALSE) taxa#natura$motus$target_taxon <- grepl("Fungi", natura$motus$path)
natura$motus$target_taxon <- grepl("k__Fungi", natura$motus$Kingdom)

# Proportion of each of these over total number of MOTUs
table(natura$motus$target_taxon) / nrow(natura$motus)

#   FALSE       TRUE 
# 0.01131648 0.98868352  #they are unassigned
