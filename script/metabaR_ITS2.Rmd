...
file_samples <- read.delim2("E:/DRYER_natura_data/ITS2/ngs_filter_ITS2.tab", header = FALSE)
...
#pcrs
ngsfilter <-read.delim2("E:/DRYER_natura_data/ITS2/ngs_filter_ITS2.tab", header = FALSE)
...
#obifile
file_obitab <- read.delim2("E:/DRYER_natura_data/ITS2/DRYER_natura_R1R2_good_demultiplexed_derepl_basicfilt_cl_agg_12042023.tab")
#colnames(file_obitab)
colnames(file_obitab) <- gsub("sample.", "", colnames(file_obitab)) # remove the pattern 'sample.' from column names

#read
colnames(file_obitab)
read <- file_obitab %>% select(c(1,21:183))
read_t <- t(read[,-1])
...
#motus
motus <- file_obitab[, c(1,20,205)] #take the sequence column
rownames(motus) <- motus$id

#does not work
assign <- read.csv("E:/DRYER_natura_data/ITS2/assigation_ITS213042023_3.csv", header = TRUE)
assign$id <- gsub(";", "", assign$id) # remove the pattern ';'
rownames(assign) <- assign$id
assign_motus <- left_join(motus, assign, by = "id") 
assign_motus<- assign_motus %>% separate(path, into = c('X', 'domain','phylum', 'class', 'order', 'family', 'genus'), sep = "%;", FALSE)

assign_motus<- assign_motus %>% separate(phylum, into = c('phylum_essai', 'score'), sep = ";", T)
assign_motus<- assign_motus %>% separate(class, into = c('class_essai', 'score'), sep = ";", T)
assign_motus<- assign_motus %>% separate(order, into = c('order_essai', 'score'), sep = ";", T)
assign_motus<- assign_motus %>% mutate(taxo_long = paste0(phylum_essai,"_", class_essai, "_", order_essai))
assign_motus<- assign_motus %>% mutate(taxo_court = paste0(phylum_essai,"_", class_essai))
rownames(assign_motus) <- assign_motus$id

summary(file_obitab$seq_length) #267.1 (range 82-485)
...
pcrs <- pcrs %>% filter(sample_id != "DRYER_natura_NA_ITS2_control_PCR_21")
pcrs <- pcrs %>% filter(sample_id != "DRYER_natura_NA_ITS2_control_extraction_5")


samples <- samples %>% filter(sample_id != "DRYER_natura_NA_ITS2_control_PCR_21")
samples <- samples %>% filter(sample_id != "DRYER_natura_NA_ITS2_control_extraction_5")


#Metabarlist
MetabarList <- metabarlist_generator(reads = read_t , #163*13255 #not okay
                                     motus = assign_motus, #13255*3 #ok
                                     pcrs = pcrs,  #163*8 #ok
                                     samples = samples) #163*11
```
...
# Plot
p <- gghill_rarefaction(natura.raref, group=material) 
```
...
#0, 0.938650307 : no contamination (all samples)       
#1, 0 
#2  0.006134969 : conta from extraction 1 
#3  0.055214724 : conta from pcr 9
...
natura$motus$target_taxon <- grepl("Fungi", natura$motus$path)

# Proportion of each of these over total number of MOTUs
table(natura$motus$target_taxon) / nrow(natura$motus)

#   FALSE        TRUE 
#0.005733685 0.994266315 #they are unassigned

# Intersection with extraction contaminant flags (not contaminant = T)
table(natura$motus$target_taxon,
      natura$motus$not_a_sterilisation_conta) #0 MOTUs as non-taget, which were already flagged as potential sterilisation conta

table(natura$motus$target_taxon,
      natura$motus$not_an_extraction_conta) #0 MOTUs as non-taget, which were already flagged as potential ext conta

table(natura$motus$target_taxon,
      natura$motus$not_a_pcr_conta) #0 MOTUs as non-taget, which were already flagged as potential pcr conta
```
...
#pcrs     1992461    13105  15445.43 6485.879  526.0543 310.4419
#samples  1992461    13105  15445.43 6485.879  526.0543 310.4419
...
ggsave(filename = "PCoA_ITS2_organ_species.jpeg", plot = species, width = 10, height = 8)
```
...
save(natura_clean, file = "./results/Metabarlist_natura_clean_ITS2.Rdata")

#both organs
write.csv(natura_clean$motus,"motus_without_contaminant_ITS2.csv")
write.csv(t(natura_clean$motus),"transposed_motus_without_contaminant_ITS2.csv")
write.csv(natura_clean$reads,"reads_without_contaminant_ITS2.csv")
write.csv(t(natura_clean$reads),"transposed_reads_without_contaminant_ITS2.csv")
write.csv(natura_clean$samples,"samples_without_contaminant_ITS2.csv")
write.csv(natura_clean$pcrs,"pcrs_without_contaminant_ITS2.csv")
write.csv(t(natura_clean$pcrs),"transposed_pcrs_without_contaminant_ITS2.csv")

#only leaves
leaf_ITS_clean <- subset_metabarlist(natura_clean, table = "samples",
                          indices = natura_clean$samples$organ == "leaf")
save(leaf_ITS_clean, file = "./results/Metabarlist_leaf_clean_ITS2.Rdata")

write.csv(leaf_metabar_clean$motus,"leaf_motus_without_contaminant_ITS2.csv")
write.csv(t(leaf_metabar_clean$motus),"leaf_transposed_motus_without_contaminant_ITS2.csv")
write.csv(leaf_metabar_clean$reads,"leaf_reads_without_contaminant_ITS2.csv")
write.csv(t(leaf_metabar_clean$reads),"leaf_transposed_reads_without_contaminant_ITS2.csv")
write.csv(leaf_metabar_clean$samples,"leaf_samples_without_contaminant_ITS2.csv")
write.csv(leaf_metabar_clean$pcrs,"leaf_pcrs_without_contaminant_ITS2.csv")
write.csv(t(leaf_metabar_clean$pcrs),"leaf_transposed_pcrs_without_contaminant_ITS2.csv")

#only roots
root_ITS_clean <- subset_metabarlist(natura_clean, table = "samples",
                          indices = natura_clean$samples$organ == "root")
save(root_ITS_clean, file = "./results/Metabarlist_root_clean_ITS2.Rdata")
write.csv(root_metabar_clean$motus,"root_motus_without_contaminant_ITS2.csv")
write.csv(t(root_metabar_clean$motus),"root_transposed_motus_without_contaminant_ITS2.csv")
write.csv(root_metabar_clean$reads,"root_reads_without_contaminant_ITS2.csv")
write.csv(t(root_metabar_clean$reads),"root_transposed_reads_without_contaminant_ITS2.csv")
write.csv(root_metabar_clean$samples,"root_samples_without_contaminant_ITS2.csv")
write.csv(root_metabar_clean$pcrs,"root_pcrs_without_contaminant_ITS2.csv")
write.csv(t(root_metabar_clean$pcrs),"root_transposed_pcrs_without_contaminant_ITS2.csv")
```
...
tmp_leaf <- tmp[grep("leaf", tmp$Group.1), ]
tmp_leaf <- melt(tmp_leaf) %>%
  rename(Genus = Group.1,
         taxon = variable,
         reads = value)

tmp_leaf_asco <- tmp_leaf[grep("Ascomycota", tmp_leaf$taxon), ]
tmp_leaf_asco <- tmp_leaf_asco %>% filter(reads > 30)
# Plot asco
ggplot(tmp_leaf_asco, aes(x=Genus, y=reads, fill=taxon)) +
    geom_bar(stat="identity") +
    labs(x=NULL, y="#reads", fill="Phyla") +
    coord_flip() + theme_bw() +
    theme(legend.position = "bottom", text = element_text(size = 20))  +
    ggtitle("Fungi - Ascomycota")

# Basidiomycetes
tmp_leaf_basi <- tmp_leaf[grep("Basidiomycota", tmp_leaf$taxon), ]
tmp_leaf_basi <- tmp_leaf_basi %>% filter(reads > 30)
ggplot(tmp_leaf_basi, aes(x=Genus, y=reads, fill=taxon)) +
    geom_bar(stat="identity") +
    labs(x=NULL, y="#reads", fill="Phyla") +
    coord_flip() + theme_bw() +
    theme(legend.position = "bottom", text = element_text(size = 20))  +
    ggtitle("Fungi - Basidiomycota")


# Plot chytridiomycota
tmp <- tmp %>% !filter(variable == "Chytridiomycota")
ggplot(tmp, aes(x=Group.1, y=value, fill=variable)) +
    geom_bar(stat="identity", fill = "#00BA38") +
    labs(x=NULL, y="#reads", fill="Phyla") +
    coord_flip() + theme_bw() +
    theme(legend.position = "bottom") +
    ggtitle("Fungi - Chytridiomycota")
...
natura_clean_class <-
  aggregate_motus(natura_clean, groups =natura_clean$motus$class_essai)

# Aggregate per Habitat/material
tmp <-
  melt(aggregate(natura_clean_class$reads, by = list(
    paste(natura_clean_class$samples$organ,
          natura_clean_class$samples$Genus,
          sep = " | ")), sum))
...
# Aggregate the metabarlist at the class level
natura_clean_phy <-
  aggregate_motus(natura_clean, groups =natura_clean$motus$phylum_essai)

natura_clean_phy_class <-
  aggregate_motus(natura_clean_phy, groups =natura_clean_phy$motus$class_essai)

# Aggregate per Habitat/material
tmp <-
  melt(aggregate(natura_clean_phy_class$reads, by = list(
    paste(natura_clean_phy$samples$organ,
          natura_clean_phy$samples$Genus,
          sep = " | ")), sum))
...
# Aggregate the metabarlist at the phylum level
natura_clean_order <-
  aggregate_motus(natura_clean, groups =natura_clean$motus$order_essai)

# Aggregate per Habitat/material
tmp <-
  melt(aggregate(natura_clean_order$reads, by = list(
    paste(natura_clean_order$samples$organ,
          natura_clean_order$samples$Genus,
          sep = " | ")), sum))
...
table %>%
  ...
  kbl(caption = "Curated data for ITS2 : number of replicates, mean number of reads and OTUs per species.", escape = FALSE) %>% #title
  ...
  save_kable(file = "./results/summary_ITS2.png") #make it bigger add zoom = 5 in save_kable
