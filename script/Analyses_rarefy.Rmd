...
# W = 0.9155, p-value = 0.000151

#not normally distributed therefore i use BoxCox tranformation
library(MASS)
bc<-boxcox(lm(leaf$samples$Shannon_rarefy~1))
(lambda <- bc$x[which.max(bc$y)])
new_model <- lm(((leaf$samples$Shannon_rarefy^lambda-1)/lambda) ~ 1)
qqnorm(new_model$residuals)
qqline(new_model$residuals)
norm_Shannon<-(leaf$samples$Shannon_rarefy^lambda-1)/lambda
leaf$samples$norm_Shannon_rarefy <-norm_Shannon

shannon.anova.leaf<-aov(norm_Shannon_rarefy ~ Name, leaf$samples)
summary(shannon.anova.leaf) #8.74e-07 ***

alpha_ITS_plot_leaf_rarefy <- leaf$samples  %>%
...
# W = 0.91114, p-value = 0.0004347

#not normally distributed therefore i use BoxCox tranformation
library(MASS)
bc<-boxcox(lm(root$samples$Shannon_rarefy~1))
(lambda <- bc$x[which.max(bc$y)])
new_model <- lm(((root$samples$Shannon_rarefy^lambda-1)/lambda) ~ 1)
qqnorm(new_model$residuals)
qqline(new_model$residuals)
norm_Shannon<-(root$samples$Shannon_rarefy^lambda-1)/lambda
root$samples$norm_Shannon_rarefy <-norm_Shannon

shannon.anova.root<-aov(norm_Shannon_rarefy ~ Name, root$samples)
summary(shannon.anova.root) #0.00349 **

alpha_ITS_plot_root_rarefy <- root$samples  %>%
   mutate(name = paste0(substr(Genus, 1, 1), ".", " ", Species) ) %>%
  ggplot() +
  aes(x = name, y = Shannon_rarefy, fill = organ) +
  geom_boxplot() +
  ...
  ylab(expression(paste("Alpha diversity / fungi")))+ #\n jump line
  theme_minimal(base_size = 20)+
  geom_text(aes(x = 3, y = 250, label = "p-value < 0.01"), color = "black", size = 6)+
  theme(axis.text.x = element_blank(), axis.title.x = element_blank(),legend.position = "none") 
...
mod <- betadisper(d = hill.beta.leaf.dist, group = as.factor(leaf$samples$organ), type = "median")
mod
...
##drivers of beta diversity (Figure 6 fungi)
...
# 
# data:  leaf$samples$Shannon
# W = 0.99177, p-value = 0.9337


shannon.anova.leaf<-aov(Shannon_rarefy ~ Name, leaf$samples)
summary(shannon.anova.leaf) #2.38e-10 ******
...
# 
# data:  root$samples$Shannon
# W = 0.98396, p-value = 0.5837

shannon.anova.root<-aov(Shannon_rarefy ~ Name, root$samples)
summary(shannon.anova.root) #0.544
...
## drivers of beta diversity (Figure 6 bacteria)
